...
{
	if(sta==0)	HAL_GPIO_WritePin(GPIOA,GPIO_PIN_7,GPIO_PIN_RESET);	//PA7置0
	else HAL_GPIO_WritePin(GPIOA,GPIO_PIN_7,GPIO_PIN_SET);	//PA7置1
}
...
{ 
  ...
  SCCB_Init();			//初始化SCCB 的IO口 
	...
	Camera_ID<<=8;
	Camera_ID|=OV5640_RD_Reg(OV5640_CHIPIDL);	//读取ID 低八位
	
	if(Camera_ID!=OV5640_ID)//如果不是OV5640的芯片，则串口打印出ID号
	{
		//printf("ID:%d\r\n",Camera_ID);
		return 1;
	}  
